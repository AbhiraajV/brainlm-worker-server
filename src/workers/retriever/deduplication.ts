 ...
 */
export async function deduplicateAndControl(
    evidence: {
    ...
    let events = dedupedEvidence.filter(e => e.source === EvidenceSource.EVENT);
    let interpretations = dedupedEvidence.filter(e => e.source === EvidenceSource.INTERPRETATION);
    let patterns = dedupedEvidence.filter(e => e.source === EvidenceSource.PATTERN);
    let insights = dedupedEvidence.filter(e => e.source === EvidenceSource.INSIGHT);
    ...
    if (recentEvents.length < config.minRecentItems) {
        const additionalRecent = await fetchRecentEvents(
            userId,
            config.minRecentItems - recentEvents.length,
            recentBoundary,
            events.map(e => e.id)
        );
    ...
    if (historicalEvents.length < config.minHistoricalItems) {
        const additionalHistorical = await fetchHistoricalEvents(
            userId,
            config.minHistoricalItems - historicalEvents.length,
            historicalBoundary,
            events.map(e => e.id)
        );
    ...
    console.log(
        `[Retriever] Step 6 complete: ${events.length} events, ${interpretations.length} interpretations, ` +
 ...
 */
function dedupeByContent(evidence: NormalizedEvidence[]): NormalizedEvidence[] {
    const seen = new Set<string>();
    const unique: NormalizedEvidence[] = [];
 ...
 */
async function dedupeByEmbeddingSimilarity(
    ...
    // Group by source
    const bySource = new Map<EvidenceSource, NormalizedEvidence[]>();
    for (const item of evidence) {
    ...
    for (const [source, items] of bySource) {
        ...
        const embeddings = await Promise.all(
            items.map(item => embedText({ text: item.content.substring(0, 8000) }))
        );
...
async function fetchRecentEvents(
    ...
    return events.map(event => ({
        ...
        retrievalReason: RetrievalReason.COVERAGE_CONTROL,
    }));
}
 ...
 */
function sortAndCap(evidence: NormalizedEvidence[], limit: number): NormalizedEvidence[] {
    return evidence
